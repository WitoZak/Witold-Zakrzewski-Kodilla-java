...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        List<Integer> numbersOfDays = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
        ...
        double averageDays = IntStream.range(0, numbersOfDays.size())
                .map(numbersOfDays::get)
                .average().getAsDouble();

        //Then
        assertEquals(3, numbersOfDays.size());
