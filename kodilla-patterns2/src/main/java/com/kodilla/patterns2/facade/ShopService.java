...
public class ShopService {
    ...
    public boolean removeItem(Long orderId, Long productId) {
        ...
        while (orderIterator.hasNext()) {
            ...
            for (int n = 0; n < theOrder.getItems().size(); n++) {
                if (theOrder.getItems().get(n).getProductId().equals(productId)) {  //brak ()
    ...
    public BigDecimal calculateValue(Long orderId) {
        ...
        while (orderIterator.hasNext()) {
            Order theOrder = orderIterator.next();
            return theOrder.calculateValue();  //literówka
        }
    ...
    public void cancelOrder(Long orderId) {
        Iterator<Order> orderIterator = orders.stream()
                ...
                .iterator();
        while (orderIterator.hasNext()) {
            Order theOrder = orderIterator.next();  //zamiast hasnext -> next
